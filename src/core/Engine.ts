import { State, Config, Sample, Block, Chrom } from '../types.js';
    ...
    static calculateScene(state: State, config: Config): { visibleSamples: Sample[] } {
        ...
        if (currentFocusKey !== lastFocusKey) {
            ...
            if (focusSet.size > 0) {
                ...
                const focusRankMap = new Map<string, number>();
                refFocusChroms.forEach((c, idx) => {
                    focusRankMap.set(c.id, idx);
                });

                const getFocusScore = (c: Chrom): number => {
                    let scoreSum = 0;
                    let weightSum = 0;
                ...
                state.samples.forEach(sample => {
                    if (sample.id === state.refGenomeId) return;
